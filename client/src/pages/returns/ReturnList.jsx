              ...
              <div style={{ padding: '12px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '13px', color: '#777', marginTop: '4px' }}>
                    ...
                    {ret.items?.map(i => `${i.product?.product_name || 'Product'} (x${parseFloat(i.return_quantity || i.quantity || 1)}${i.unit_name ? ` ${i.unit_name}` : ''}) - ${i.action || 'REFUND'}`).join(', ')}
                  </div>
                </div>
                ...
                <tbody>
                  {selectedReturn.items?.map((item) => (
                    <tr key={item.return_item_id} style={{ borderTop: '1px solid #eee' }}>
                      ...
                      <td style={{ padding: '8px 12px' }}>{parseFloat(item.return_quantity || item.quantity || 1)} {item.unit_name || ''}</td>
