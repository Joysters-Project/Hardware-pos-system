      ...
      if (!r.items || r.items.length === 0) {
        ...
        r.items.forEach((item) => {
          csvRows.push([
            ...
            fmt(remainingBalance),
            item.product?.product_name || `Product #${item.product_id}`,
            `${parseFloat(item.return_quantity || item.quantity || 0)}${item.unit_name ? ` ${item.unit_name}` : ''}`,
  ...
  const handleDownloadReturnsPdf = () => {
    ...
    const pdfRows = filteredReturns.map((r) => {
      ...
      const productsStr = r.items && r.items.length > 0
        ? r.items.map(item => `${item.product?.product_name || `Product #${item.product_id}`} (${parseFloat(item.return_quantity || item.quantity || 0)}${item.unit_name ? ` ${item.unit_name}` : ''})`).join(', ')
        : '–';
                                    ...
                                    <tbody>
                                      {record.items.map((item, idx) => (
                                        <tr key={item.return_item_id || idx}>
                                          <td className="muted">{idx + 1}</td>
                                          <td className="bold">{item.product?.product_name || `Product #${item.product_id}`}</td>
                                          <td className="center">{parseFloat(item.return_quantity || item.quantity || 0)} {item.unit_name || ''}</td>
