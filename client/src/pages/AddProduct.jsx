			...
			<form className="add-product-form" onSubmit={handleSubmit}>
				...
				<div className="form-section">
					...
					{alternativeUnits.map((item, idx) => {
						...
						const baseUnit = units.find(u => String(u.unit_id) === String(form.unit_id));
						const hasRelationship = selectedAltUnit && baseUnit && item.conversion_factor;

						return (
							<div key={idx} className="alt-unit-row" style={{ border: "1px solid #efe2e2", padding: "20px 16px 16px", borderRadius: "12px", marginBottom: "16px", background: "#fcf9f9", position: "relative" }}>
								{hasRelationship && (
									<div style={{
										display: "inline-flex",
										alignItems: "center",
										...
										{form.cost_price && (
											<span style={{ color: "#4b5563", fontWeight: 400, marginLeft: "6px" }}>
												(Standard Cost: Rs. {(parseFloat(form.cost_price || 0) * parseFloat(item.conversion_factor || 1)).toFixed(2)})
											</span>
										)}
									</div>
								)}
								...
								<div className="form-grid">
									...
									<div className="field-group">
										<label>Cost Price for Unit</label>
